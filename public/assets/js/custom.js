...
function loadEventDraggable() {
    $(document)
        .find("#external-events .fc-event-new")
        .each(function () {
...
function showAcceptJobPopup(info) {
    ...
    let data = {
        eventStartDate: moment(info.event._instance.range.start).format(
            "YYYY-MM-DD HH:mm:ss"
        ),
        eventEndDate: moment(info.event._instance.range.end).format(
            "YYYY-MM-DD HH:mm:ss"
        ),
        truckId: info.event._def.resourceIds[0],
    };
...
function __callAcceptJobPopupScheduler(url, paramData) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#acceptJobSchedulerBtn").modal("show");
...
function generateEventHtml(eventEl) {
    ...
    return (
        ...
        '<div class="pending_job_section"><div class="pending_top d-flex align-items-center"> <span>' +
        customerName +
        "</span><p>Job " +
        id +
        '<span class="light_color">(5 hours)</span></p></div><div class="job_description">' +
        '<div class="address_content"><p class="active">' +
        startAddr +
        ", " +
        startCity +
        "</p><p>" +
        ...
        ", " +
        endCity +
        '</p></div></div></div><div class="chart_job_price"> <span class="price"><sup>$</sup>' +
        totalPrice +
        "</span> <span>View details</span></div></div>"
    );
}
...
$(function () {
    ...
    $(document).on(
        "click",
        ".move__company__active__inactive",
        function (event) {
    ...
    req.onload = function () {
        ...
        if (json.routes !== undefined) {
            ...
            if (typeof rootElement != "undefined" && rootElement != null) {
                document.getElementById("route").value = JSON.stringify(
...
function calendarRenderAll(CalendarId) {
    ...
    console.log(" working calendarRenderAll");

    document
        .querySelectorAll(".fc-time-area .fc-rows tr")
        .forEach(function (trElement) {
            var childCountActualCount = 0;
            trElement
                .querySelectorAll(".fc-event-container > a")
                .forEach(function (move) {
                    countHeight = childCountActualCount * 100 + 100;
                    move.style.marginTop =
                        (countHeight - 100).toString() + "px";
                    move.parentElement.parentElement.style.height =
                        countHeight.toString() + "px";

                    var containerDiv = move.parentElement.parentElement;
                    if (
                        typeof containerDiv.parentElement !== "undefined" &&
                        typeof containerDiv.parentElement.parentElement !==
                            "undefined"
                    ) {
                        var divResourceId =
                            containerDiv.parentElement.parentElement.getAttribute(
                                "data-resource-id"
                            );
                        var resourceElement = document.querySelector(
                            ".fc-resource-area .fc-rows > table tr:nth-child(" +
                                parseInt(divResourceId) +
                                ") > td > div"
                        );

                        if (resourceElement !== null) {
                            resourceElement.style.height =
                                countHeight.toString() + "px";
                        }
                    }
