                        ...
                        <table align="center" border="0" cellpadding="0" cellspacing="0" class="container590" width="590">
                            <tr>
                                <td align="left" style="color: #888888; font-size: 16px; font-family: 'Work Sans', Calibri, sans-serif; line-height: 24px;">
                                    <p style="line-height: 24px; margin-bottom:15px;">{{ $move->customer->name }},</p>
                                    <p style="line-height: 24px;margin-bottom:15px;"> Thank you for your interest in the Biggles Service. At this moment, we are not able to proceed your request because of some transport issue. Once that available will get back to you!</p>
                                    <p><b>Reason to Decline Job: </b> {{ $reason }}</p><br><br>
                                    <p style="line-height: 24px; margin-bottom:20px;"> Below are the detail from your Booking:</p>
                                    <p><b>Customer Name:</b> {{ $move->customer->name }}</p>
                                    <p><b>Company Name:</b> {{ $move->company->name }}</p>
                                    <p><b>Start Address:</b> {{ $move->start_addr }}</p>
                                    <p><b>End Address:</b> {{ $move->end_addr }}</p>
                                    <p><b>Booked Date:</b> {{ !is_null($move->created_at) && !empty($move->created_at) ? Carbon\Carbon::parse($move->created_at)->format('d M Y - H:i') : '' }}</p>
                                    <br /><br />
