...
class MoveController extends Controller
{
    ...
    */
     ...
     * @return Response
     */
    public function getConfigurationItem(Request $request)
    {
        // Get the config Items
        $bedRoomConfigs = array_keys(Config::get('inventoryList.Bedroom'));
        $loungeConfigs  = array_keys(Config::get('inventoryList.Lounge'));
        $kitchenConfigs = array_keys(Config::get('inventoryList.Kitchen'));
        $laundryConfigs = array_keys(Config::get('inventoryList.Laundry'));
        $officeConfigs  = array_keys(Config::get('inventoryList.Office'));
        $hallConfigs    = array_keys(Config::get('inventoryList.Hall'));
        $garageConfigs  = array_keys(Config::get('inventoryList.Garage'));
        $cartonsConfigs = array_keys(Config::get('inventoryList.Cartons'));

        // return the config items for volume calculations
        return response()->json(["Bedroom" => $bedRoomConfigs,
                                 "Lounge"  => $loungeConfigs,
                                 "Kitchen" => $kitchenConfigs,
                                 "Laundry" => $laundryConfigs,
                                 "Office"  => $officeConfigs,
                                 "Hall"    => $hallConfigs,
                                 "Garage"  => $garageConfigs,
                                 "Cartons" => $cartonsConfigs,
